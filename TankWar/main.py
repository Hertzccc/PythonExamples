...
def show_start_interface(screen, width, height):
    tfont = pygame.font.Font('./font/simkai.ttf', width//4)  # 字体大小不一样
    cfont = pygame.font.Font('./font/simkai.ttf', width//20)
    title = tfont.render(u'坦克大战', True, (255, 0, 0))     # 最后参数是颜色
    content1 = cfont.render(u'按1键进入单人游戏', True, (0, 0, 255))
    content2 = cfont.render(u'按2键进入双人人游戏', True, (0, 0, 255))
    trect = title.get_rect()  # trect: title rectangle
    trect.midtop = (width/2, height/4)  # 获取大小，调整位置
    crect1 = content1.get_rect()
    crect1.midtop = (width/2, height/1.8)
    crect2 = content2.get_rect()
    crect2.midtop = (width/2, height/1.6)
    screen.blit(title, trect)  # 相当于add
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                sys.exit()
            elif event.type == pygame.KEYDOWN:   # 双人游戏或单人游戏
...
def show_switch_stage(screen, width, height, stage):
    ...
    screen.blit(bg_img, (0, 0))
    font = pygame.font.Font('./font/simkai.ttf', width//10)
    content = font.render(u'第%d关' % stage, True, (0, 255, 0))  # 根据传参设定关卡数
    rect = content.get_rect()
    rect.midtop = (width/2, height/2)
    screen.blit(content, rect)
    pygame.display.update()
    delay_event = pygame.constants.USEREVENT + 3  # 第三个event
...
def handle_mytanksGroup(stage_data, screen):
    if stage_data.tank_player1 in stage_data.mytanksGroup:       # 是 mytanksGroup， 我方玩家
        # 当玩家按下方向键, 即坦克在移动时, 通过切换两张不同的外观, 来显示坦克履带的运动的特效，
        # 动的时候切tank_0，不动的时候切tank_1, 后面dest位置传的参数都一样，都是tank此时的rect.left 和 rect.top
        if stage_data.is_switch_tank and stage_data.player1_moving:
            ...
            screen.blit(stage_data.tank_player1.tank_1,
                        (stage_data.tank_player1.rect.left, stage_data.tank_player1.rect.top))
        if stage_data.tank_player1.protected:  # 有保护罩
            screen.blit(stage_data.tank_player1.protected_mask1,
                        (stage_data.tank_player1.rect.left, stage_data.tank_player1.rect.top))
    if stage_data.num_player > 1:  # 两个玩家处理
        if stage_data.tank_player2 in stage_data.mytanksGroup:
            if stage_data.is_switch_tank and stage_data.player2_moving:
                ...
                screen.blit(stage_data.tank_player2.tank_1,
                            (stage_data.tank_player2.rect.left, stage_data.tank_player2.rect.top))
            if stage_data.tank_player2.protected:  # 这个应该是都用了一个保护罩对象
                screen.blit(stage_data.tank_player1.protected_mask1,
                            (stage_data.tank_player2.rect.left, stage_data.tank_player2.rect.top))
...
def handle_enemytanksGroup(stage_data, screen):
    for each in stage_data.enemytanksGroup:  # 每一个敌方坦克对象
        # 出生特效
        if each.born:
            if each.times > 0:
                each.times -= 1  # times 是 90
                # 星星从小到大有3张图片, 每10个滴答改变一次外观, 90个滴答内, 共可显示3遍从小到大变化的特效
                # each.x 是敌方坦克随机出现的三个位置，坦克长48，3个出生点共长48*3=24*6，一行24*26
                if each.times <= 10:
                    screen.blit(
    ...
    for tank_player in stage_data.mytanksGroup:
        if tank_player.bullet.being:
            ...
            for each in stage_data.enemybulletsGroup:
                if each.being:  # 用这个 collide_rect检测碰撞，tank_player是我方，each是敌方
                    if pygame.sprite.collide_rect(tank_player.bullet, each):
                        tank_player.bullet.being = False  # 会抵消，两个子弹being都是false，然后抹除敌方子弹
                        each.being = False
                ...
                if each.being:
                    if pygame.sprite.collide_rect(tank_player.bullet, each):
                        ...
                        if each.is_red:
                            myfood = food.Food()
                            myfood.generate()
                            stage_data.myfoodsGroup.add(myfood)  # 添加食物
                            each.is_red = False 	# 击中后不再是红色
                        # 敌方坦克被击中后, 血量变少, 坦克颜色也随之改变
                        each.blood -= 1
                        each.color -= 1
                        if each.blood < 0:
                            sound_data.bang_sound.play() 	# 击杀音效
                            each.being = False
                            stage_data.enemytanksGroup.remove(each)
                            stage_data.enemytanks_now -= 1		# 敌方存活数量-1
                            stage_data.tanksGroup.remove(each)  # 总坦克组 里也得删掉
                        else:
                            each.reload()  # 这个没有查到
                        tank_player.bullet.being = False  # 子弹生命结束
                        break
                else:  # 检测到不活了，直接从组里删掉
                    stage_data.enemytanksGroup.remove(each)
                    stage_data.tanksGroup.remove(each)
            # 子弹碰撞石头墙。
                    # #spritecollide()是一个小精灵和一个组碰撞检测，与上面collide_rect 检测两个小精灵不相同。 后面true表示要删除group中产生碰撞的对象
            ...
            # 子弹碰钢墙
            if tank_player.bullet.stronger:  # 牛逼的子弹可以穿墙
                        ...
                        if pygame.sprite.collide_rect(each.bullet, tank_player):
                            ...
                            if not tank_player.protected:
                                ...
                                if tank_player.life < 0:
                                    stage_data.mytanksGroup.remove(
                                        tank_player)  # 删掉这个挂掉的玩家
...
def handle_food(stage_data, sound_data, screen):
    for myfood in stage_data.myfoodsGroup:
        if myfood.being and myfood.time > 0:  # food 还能捡
            screen.blit(myfood.food, myfood.rect)  # 渲染food
            myfood.time -= 1
            for tank_player in stage_data.mytanksGroup:
                # 当玩家的坦克碰到food时
                if pygame.sprite.collide_rect(tank_player, myfood):  # 下面对应不同的food效果
                    # 消灭当前所有敌人，最牛逼的
                    if myfood.kind == 0:
                        ...
                        stage_data.enemytanksGroup = pygame.sprite.Group()  # 对象被赋空值
                    ...
                    if myfood.kind == 1:
                        ...
                        for each in stage_data.enemytanksGroup:
                            each.can_move = False
                        pygame.time.set_timer(  # 设置millis = 8000
...
def handle_player1_events(key_pressed, sound_data, stage_data):
    ...
    if key_pressed[pygame.K_w]:  # 每一个都是先删掉，更新，再加入
    ...
    if key_pressed[pygame.K_SPACE]:  # 射击的时候 有声音，调用射击
...
def handle_player2_events(key_pressed, sound_data, stage_data):
    if stage_data.num_player > 1:
        ...
        if key_pressed[pygame.K_RETURN]:  # 因为我的键盘没有小键盘，所以改成return
...
# 用于存放声音相关的数据
class SoundData:
    def __init__(self):  # 包括了 add_sound, bang_sound, blast_sound, fire_sound, Gunfire_sound, hit_sound, start_sound;
...
class StageData:
    def __init__(self, num_player, stage):
        ...
        # 精灵组 有五个个，所有坦克组，我方坦克组，敌方坦克组，敌方子弹组，我方食物组；
        ...
        # 自定义事件 这里有三个 ，生成敌方坦克， 敌方坦克静止恢复，我方坦克无敌恢复
        # 	-生成敌方坦克事件 用户事件 - generate enemy tank
        ...
        # 游戏玩家是否按下方向键，初始化为false。真动了，会变成true。在监听事件中，会将true变成false
