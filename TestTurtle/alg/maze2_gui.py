...
GUI_X_MAX = alg.maze2.X_MAX + 1
GUI_Y_MAX = alg.maze2.Y_MAX + 1
GUI_X_MIN = 0
GUI_Y_MIN = 0
...
def repaint():
    #  如果迷宫算法还没有结束
    if not alg.maze2.HAS_FINISHED:
        # 关闭海龟动画
        my_screen.tracer(0)
        # 运行迷宫算法，只探索一步
        alg.maze2.explore(alg.maze2.map, True)
        #
        n = len(alg.maze2.stack)
        if n >= 2:
            i = 2
        elif n == 1:
            i = 1
